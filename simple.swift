...
app (file fmesh, file ferr, file fout, file salPort) makeMeshWritePort (file runSalomeScript, 
                                                                  file meshScript, file utils[], file fsimParams) {
    bash filename(runSalomeScript)  filename(salPort) filename(meshScript) filename(fsimParams)
...
foreach fsimParams,i in simFileParams{
    ...
    (fmesh, salomeErr, salomeOut, fsalPortNum) = makeMeshWritePort(runSalomeScript, meshScript, utils,
                                                                   fsimParams);
    fmeshes[i] = fmesh;
